...
export class RNTween extends React.Component {
	constructor(props) {
		...
		if (configNames.length > 0) {
			...
			if (config) {
				this.state.animatedStyles = this._getComputedAnimatedStyles(config);
			}
		}
	...
	_createAnimatedTween(config, reversed) {
		...
		const createTiminingTween = () =>
			Animated.timing(value, {
				...
				useNativeDriver:
					useNative === null || useNative === undefined ? true : useNative,
		...
		for (let k in config) {
			...
			newConfig.configs = c.configs
				...
				.map(e => {
					return {
						...e,
						value: new Animated.Value(typeof e.from === 'string' ? 0 : e.from),
